...
namespace MathUtils {
    ...
    // Dinamik asal sayı üretici (sınırsız)
    bool is_prime(u64 n) {
        if (n <= 1) return false;
        if (n <= 3) return true;
...
private:
    ...
    size_t calculate_optimized_block_size(size_t block_index, size_t remaining_data, size_t total_blocks) const {
        ...
        u64 fib_val = MathUtils::fibonacci(block_index + 2);   // Sınırsız Fibonacci
        u64 prime_val = MathUtils::nth_prime(block_index + 1); // Sınırsız n. prime
...
int main() {
    ...
    if (all_correct) cout << "✓ Tüm sorgular doğru!\n";
    else cout << "✗ Toplam " << error_count << " hata bulundu!\n";
